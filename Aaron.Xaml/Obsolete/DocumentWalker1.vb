'Imports System.Windows.Documents
'Imports System.Windows.Controls

' ''' <summary>
' ''' THe delegate type of the event that will be raised
' ''' </summary>
'Public Delegate Sub DocumentVisitedEventHandler(sender As Object, visitedObject As Object, start As Boolean)

' ''' <summary>
' ''' The Document walker class enables a traversal of the flow document tree and raises an event for each node 
' ''' in the document tree. I used it to find all instances of a FormattedRun control in the flowdocument defintion. 
' ''' It is pretty straightforward, so I will just show the code itself. 
' ''' </summary>
'Public Class DocumentWalker1
'	Protected Sub New()
'	End Sub

'	Shared Function Use() As DocumentWalker1
'		Return New DocumentWalker1
'	End Function



'	' ''' <summary>
'	' ''' Gets or sets the tag associated to this walker
'	' ''' </summary>
'	'Public Property Tag As Object


'	''' <summary>
'	''' This is the event to hook on.
'	''' </summary>
'	Public Event VisualVisited As DocumentVisitedEventHandler

'	''' <summary>
'	''' Traverses whole document
'	''' </summary>
'	''' <param name="fd">FlowDocument</param>
'	''' <returns>list of inlines</returns>
'	Public Function Walk(fd As FlowDocument) As List(Of Inline)
'		Return TraverseBlockCollection(Of Inline)(fd.Blocks)
'	End Function

'	''' <summary>
'	''' Traverses whole document
'	''' </summary>
'	''' <param name="fd">FlowDocument</param>
'	''' <returns>list of inlines</returns>
'	Public Function Walk(Of T As Class)(fd As FlowDocument) As List(Of T)
'		Return TraverseBlockCollection(Of T)(fd.Blocks)
'	End Function

'	''' <summary>
'	''' Traverses an InlineCollection
'	''' </summary>
'	''' <param name="inlines">InlineCollection to be traversed</param>
'	''' <returns>list of inlines</returns>
'	Public Function TraverseInlines(Of T As Class)(inlines As InlineCollection) As List(Of T)
'		Dim res As New List(Of T)()
'		If inlines IsNot Nothing AndAlso inlines.Count > 0 Then
'			Dim il As Inline = inlines.FirstInline
'			While il IsNot Nothing
'				If TypeOf il Is T Then
'					res.Add(TryCast(il, T))
'				End If

'				Dim r As Run = TryCast(il, Run)
'				If r IsNot Nothing Then
'					RaiseEvent VisualVisited(Me, r, True)
'					il = il.NextInline
'					Continue While
'				End If


'				Dim sp As Span = TryCast(il, Span)
'				If sp IsNot Nothing Then
'					RaiseEvent VisualVisited(Me, sp, True)

'					res.AddRange(TraverseInlines(Of T)(sp.Inlines))
'					il = il.NextInline
'					Continue While
'				End If

'				Dim uc As InlineUIContainer = TryCast(il, InlineUIContainer)
'				If uc IsNot Nothing AndAlso uc.Child IsNot Nothing Then
'					RaiseEvent VisualVisited(Me, uc.Child, True)
'					If TypeOf uc.Child Is T Then
'						res.Add(TryCast(uc.Child, T))
'					End If
'					Dim tb As TextBlock = TryCast(uc.Child, TextBlock)
'					If tb IsNot Nothing Then
'						res.AddRange(TraverseInlines(Of T)(tb.Inlines))
'					End If

'					il = il.NextInline
'					Continue While
'				End If
'				Dim fg As Figure = TryCast(il, Figure)
'				If fg IsNot Nothing Then
'					RaiseEvent VisualVisited(Me, fg, True)
'					res.AddRange(TraverseBlockCollection(Of T)(fg.Blocks))
'				End If
'				il = il.NextInline
'			End While
'		End If
'		Return res
'	End Function


'	''' <summary>
'	''' Traverses only passed paragraph
'	''' </summary>
'	''' <param name="p">paragraph</param>
'	''' <returns>list of inlines</returns>
'	Public Function TraverseParagraph(Of T As Class)(p As Paragraph) As List(Of T)
'		Return TraverseInlines(Of T)(p.Inlines)
'	End Function

'	''' <summary>
'	''' Traverses passed block collection
'	''' </summary>
'	''' <param name="blocks">blocks to be traversed</param>
'	''' <returns>list of inlines</returns>
'	'''    
'	Public Function TraverseBlockCollection_1(Of T As Class)(blocks As IEnumerable(Of Block)) As List(Of T)
'		Dim res As New List(Of T)()
'		For Each b As Block In blocks
'			If TypeOf b Is T Then
'				RaiseEvent VisualVisited(Me, b, True)
'				res.Add(TryCast(b, T))
'			End If

'			Dim p As Paragraph = TryCast(b, Paragraph)
'			If p IsNot Nothing Then
'				RaiseEvent VisualVisited(Me, p, True)
'				res.AddRange(TraverseParagraph(Of T)(p))
'				Continue For
'			End If

'			Dim bui As BlockUIContainer = TryCast(b, BlockUIContainer)
'			If bui IsNot Nothing Then
'				RaiseEvent VisualVisited(Me, bui.Child, True)
'				Continue For
'			End If

'			Dim s As Section = TryCast(b, Section)
'			If s IsNot Nothing Then
'				RaiseEvent VisualVisited(Me, s, True)
'				res.AddRange(TraverseBlockCollection(Of T)(s.Blocks))
'				Continue For
'			End If

'			Dim TT As Table = TryCast(b, Table)
'			If TT IsNot Nothing Then
'				RaiseEvent VisualVisited(Me, TT, True)
'				For Each trg As TableRowGroup In TT.RowGroups
'					RaiseEvent VisualVisited(Me, trg, True)
'					For Each tr As TableRow In trg.Rows
'						RaiseEvent VisualVisited(Me, tr, True)
'						If TypeOf tr Is T Then
'							res.Add(TryCast(tr, T))
'						End If

'						For Each tc As TableCell In tr.Cells
'							RaiseEvent VisualVisited(Me, tc, True)
'							res.AddRange(TraverseBlockCollection(Of T)(tc.Blocks))
'						Next
'					Next
'				Next
'				Continue For
'			End If
'		Next
'		Return res
'	End Function



'	''' <summary>
'	''' Traverses passed block collection
'	''' </summary>
'	''' <param name="blocks">blocks to be traversed</param>
'	''' <returns>list of inlines</returns>
'	'''    
'	Public Function TraverseBlockCollection(Of T As Class)(blocks As IEnumerable(Of Block)) As List(Of T)
'		Dim res As New List(Of T)
'		For Each b As Block In blocks
'			If TypeOf b Is T Then
'				RaiseEvent VisualVisited(Me, b, True)
'				res.Add(TryCast(b, T))
'			End If

'			Select Case b.GetType
'				Case GetType(Paragraph)
'					Dim p1 As Paragraph = b
'					RaiseEvent VisualVisited(Me, p1, True)
'					res.AddRange(TraverseParagraph(Of T)(p1))
'				Case GetType(BlockUIContainer)
'					Dim bui1 As BlockUIContainer = b
'					RaiseEvent VisualVisited(Me, bui1.Child, True)
'				Case GetType(Section)
'					Dim s1 As Section = b
'					RaiseEvent VisualVisited(Me, s1, True)
'					res.AddRange(TraverseBlockCollection(Of T)(s1.Blocks))
'				Case GetType(Table)
'					Dim TT As Table = b
'					RaiseEvent VisualVisited(Me, TT, True)
'					For Each trg As TableRowGroup In TT.RowGroups
'						RaiseEvent VisualVisited(Me, trg, True)
'						For Each tr As TableRow In trg.Rows
'							RaiseEvent VisualVisited(Me, tr, True)
'							If TypeOf tr Is T Then
'								res.Add(TryCast(tr, T))
'							End If

'							For Each tc As TableCell In tr.Cells
'								RaiseEvent VisualVisited(Me, tc, True)
'								res.AddRange(TraverseBlockCollection(Of T)(tc.Blocks))
'							Next
'						Next
'					Next
'			End Select
'		Next
'		Return res
'	End Function

'End Class
